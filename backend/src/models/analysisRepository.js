import { Analysis } from './Analysis.js';

export class AnalysisRepository {
  async list(userId) {
    try {
      const list = await Analysis.find(userId ? { userId } : {}, {
        'result.profiling': 0,
        'result.quality_checks': 0,
        'result.model_summary': 0,
        'result.explanation': 0,
        'result.explainability': 0,
        'result.explanation_summary': 0,
        'result.report_markdown': 0,
        'result.analysis_log': 0,
        'result.corrected_csv': 0,
        'result.proxy_detection': 0,
        'result.root_causes': 0,
        'result.recommendations': 0,
        'result.mitigation_summary.candidates': 0,
        'result.corrected_sensitive_findings': 0,
        'result.corrected_intersectional_findings': 0,
        'result.sensitive_findings.notes': 0,
        'result.sensitive_findings.group_metrics': 0,
        'result.intersectional_findings.group_metrics': 0,
        'result.intersectional_findings.notes': 0,
        'mitigationPreview': 0,
        'result.artifacts.corrected_csv': 0,
        'result.artifacts.fairness_report_markdown': 0,
        'result.artifacts.pdf_report_path': 0,
        'artifactPaths': 0
      })
      .sort({ createdAt: -1 })
      .limit(25)
      .lean()
      .maxTimeMS(2500);
      return list;
    } catch (err) {
      console.error('Mongoose list error:', err.message);
      return [];
    }
  }

  async getById(id, userId) {
    try {
      const doc = await Analysis.findOne(userId ? { id, userId } : { id }).lean().maxTimeMS(5000);
      return doc;
    } catch (err) {
      console.error(`Mongoose getById(${id}) error:`, err.message);
      return null;
    }
  }

  async save(analysis) {
    try {
      const doc = await Analysis.findOneAndUpdate(
        { id: analysis.id },
        { $set: analysis },
        { new: true, upsert: true, maxTimeMS: 5000 }
      ).lean();
      return doc;
    } catch (err) {
      console.error('Mongoose save error:', err.message);
      return analysis;
    }
  }

  async deleteById(id, userId) {
    try {
      const doc = await Analysis.findOneAndDelete(userId ? { id, userId } : { id }).lean().maxTimeMS(5000);
      return doc;
    } catch (err) {
      console.error(`Mongoose deleteById(${id}) error:`, err.message);
      return null;
    }
  }
}
