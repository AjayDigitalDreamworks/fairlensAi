...
def run_audit(
    ...
    if not quality["suitable"]:
        return {
            "metadata": {"rows": int(len(df)), "columns": df.columns.tolist(), "source_name": source_name, "target_column": inference.target_column, "sensitive_columns": inference.sensitive_columns, "domain": domain or "auto"},
            "errors": quality["errors"],
            "warnings": quality["warnings"] + inference.warnings + fallback_warnings,
            "profiling": inference.profiling,
            "recommendation": "Dataset unsuitable for reliable fairness audit.",
            "report_markdown": "Dataset unsuitable for reliable fairness audit.",
        }
    ...
    if pred_column == inference.target_column:
        pred_column = None
        prob_column = None
    if not pred_column and inference.target_column:
        try:
