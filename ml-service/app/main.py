...
GLOBAL_SHAP_FEATURE_LIMIT = 8
LOCAL_EXPLANATION_LIMIT = 3
LOCAL_CONTRIBUTOR_LIMIT = 4
GEMINI_MODEL = os.getenv("FAIRAI_GEMINI_MODEL", "gemini-2.5-flash")
GEMINI_API_BASE = os.getenv("FAIRAI_GEMINI_API_BASE", "https://generativelanguage.googleapis.com/v1beta")
...
def generate_supervised_predictions(
    ...
    probabilities, reweighing_summary, bundle = fit_binary_xgboost_bundle(
        X,
        y,
        sensitive_columns,
...
def fit_binary_xgboost_bundle(
    ...
    return probabilities, reweighing_summary, {
        "pipeline": model,
...
def build_prediction_explainability_bundle(
    ...
    if is_probability_like(df[prediction_column]):
        mimic_target = (pd.to_numeric(df[prediction_column], errors="coerce").fillna(0.0) >= 0.5).astype(int)
    else:
        mimic_target = normalize_binary(df[prediction_column], positive_label)

    if mimic_target.nunique(dropna=False) < 2:
        return None
    ...
    if target_column:
        feature_exclusions.add(target_column)

    feature_frame = df.drop(columns=[column for column in feature_exclusions if column in df.columns], errors="ignore")
    if feature_frame.empty:
        return None

    probabilities, _, bundle = fit_binary_xgboost_bundle(
        feature_frame,
        mimic_target,
        sensitive_columns,
        sample_target_name="__prediction_target__",
        model_source="xgboost_prediction_surrogate",
        explanation_basis=f"Trained to mimic prediction column '{prediction_column}' for SHAP-based attribution.",
        apply_reweighing=False,
    )
    bundle["prediction_scores"] = probabilities
    bundle["prediction_target_column"] = prediction_column
    bundle["positive_label"] = str(positive_label)
    return bundle
...
def build_tree_shap_explainability(
    ...
    return {
        ...
        "note": (
            "TreeSHAP values were computed directly from the XGBoost analysis model."
            if model_source == "xgboost_training_model"
            else model_bundle.get("explanation_basis", "TreeSHAP values were computed from an XGBoost surrogate explainer.")
        ),
    }
